...
// Function: Reset all variables to base values (INCOMPLETE)
...
const winCombo = ["PAIR OF JACKS+", "2 PAIRS", "3 OF A KIND", "STRAIGHT", "FLUSH", "FULL HOUSE", "4 OF A KIND", "STRAIGHT FLUSH", "ROYAL FLUSH"];
...
const buildBoard = (board) => {
  ...
  for (let i=0; i<board.length; i +=1) {
    ...
    if (i%2 === 1) {
      cell.classList.add('cell-darkblue', 'long');
    } else {
      cell.classList.add('cell-lightblue', 'long');
    }
    cell.innerHTML = winCombo[i]
  ...
  for (let i = 0; i < board.length; i += 1) {
    ...
    for (let j = 0; j < row.length; j += 1) {
      ...
      const cell = document.createElement('div');
      if (i%2 === 1) {
        cell.classList.add('cell-darkblue');
      } else {
        cell.classList.add('cell-lightblue');
      }
  ...
  for (i=0; i<playerArray.length; i+=1) {
    ...
    if (
      ...
      if(holdArray.includes(i) === false) {
        holdArray.push(i)
        parentDiv.children[0].innerText = "HELD";
      } else {
...
const calcHandScore = () => {
  ...
  if (suitArray.length === 1 && checkStraight(rankArray) === true) {
    ...
    return;
  } else if (suitArray.length === 2 && Object.values(cardRankTally).some((value) => value === 4)) {
    points = 25*chosenBet; // four of a kind
    combo = 'FOUR OF A KIND';
    return;
  } else if (suitArray.length === 2 && Object.values(cardRankTally).some((value) => value === 3)) {
    ...
    return;
  } else if (Object.values(cardRankTally).some((value) => value === 2) && rankArray.length === 3) {
    points = 2*chosenBet; // two pairs
    combo = 'TWO PAIRS';
    return;
  } else if (Object.values(cardRankTally).some((value) => value === 2) && (getKeyByValue(cardRankTally, 2) > 10 || getKeyByValue(cardRankTally,2) === 1)) {
...
const resetGame = () => {
  ...
  chosenBet = 0; 
  pointsDeductedAlr = 0;
  setTimeout(() => {output("Place your bet")}, 3000);
  displayWallet();
  displayBet();
...
const deal = () => {
  if (gameMode === "deal") {
    ...
    for (i=0; i<5; i+=1) {
      document.getElementsByClassName("card-back")[i].style.zIndex = "-1";
      document.getElementsByClassName("card")[i].removeEventListener("click", holdCard);
    };
    // tabulate points won
    createTally();
    calcHandScore();
